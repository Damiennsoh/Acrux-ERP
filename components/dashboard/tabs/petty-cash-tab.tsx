...
export function PettyCashTab() {
  ...
  const { data: pettyCash, isLoading, mutate } = useCollection('petty_cash', user?.organizationName);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createOrUpdateDoc('petty_cash', docId, data, user!.id, !!editingId);
      mutate();
      setOpen(false);
      toast.success(editingId ? 'Petty cash updated' : 'Petty cash added');
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDocWithSync('petty_cash', id, user!.id);
      mutate();
      toast.success('Petty cash deleted');
    } catch (error) {
      toast.error('Failed to delete');
    }
  };

  const filtered = (pettyCash || []).filter((p: any) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Petty Cash</h2>
        ...
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent aria-describedby={undefined} className="max-h-[90vh] overflow-y-auto sm:max-w-[500px]">
              <DialogHeader>
                <DialogTitle>{editingId ? 'Edit Petty Cash' : 'New Petty Cash Record'}</DialogTitle>
              </DialogHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Input value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="e.g. Lunch for team" />
                </div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-10 text-center text-muted-foreground">No petty cash records found</CardContent></Card>
      ) : (
