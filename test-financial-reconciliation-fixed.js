/**
 * Fixed Financial Reconciliation Test Runner
 * Corrected concurrent device updates logic
 */
...
async function runFinancialReconciliationTests() {
  ...
  // Test 6: FIXED Concurrent device updates (CORRECTED LOGIC)
  console.log('\n6️⃣ Testing concurrent device updates (FIXED)...');
  try {
    const devices = [
      { name: 'device-a', increment: 50 },
      { name: 'device-b', increment: 100 },
      { name: 'device-c', increment: 150 },
    ];
    
    let currentValue = 1000;
    const deviceResults = [];

    // CORRECTED: Each device adds their specific increment to the current value
    for (const device of devices) {
      const operation = financialOps.createFinancialOperation({
        ...
        newValue: currentValue + device.increment,
        operation: 'add',
        currency: 'USD',
        userId: device.name,
      });
  ...
  // Summary
  console.log('\n' + '='.repeat(60));
  console.log('📊 FIXED FINANCIAL RECONCILIATION TEST RESULTS');
  console.log('='.repeat(60));
  ...
  console.log('✅ FIXED: Concurrent device updates now work correctly');
